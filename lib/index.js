...
//
    ...
    */
     ...
     */
    Workbook.prototype.substitute = function(sheetName, substitutions, maxRow = -1) {
        var self = this;
        ...
        sheetData.findall("row").forEach(function(row, row_index) {
            row.attrib.r = currentRow = self.getCurrentRow(row, totalRowsInserted);
            
            if(maxRow > 0 && row_index >= maxRow){
                return;
            }
            rows.push(row);
            ...
            row.findall("c").forEach(function(cell, col_index) {
                var appendCell = true;
        ...
        if(dimension) {
            if(totalRowsInserted > 0 || totalColumnsInserted > 0) {
                ...
                console.log('dimensionEndRef:',dimensionEndRef);

                dimensionEndRef.row = rows.length;
    ...
    Workbook.prototype.loadTables = function(sheet, sheetFilename) {
        ...
        sheet.findall("tableParts/tablePart").forEach(function(tablePart, table_index, tableParts) {
            ...
            console.log('tableParts:',tableParts.length);
                if(table_index <= 20 ){
                tables.push({
